...
class DetailSearchActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val getSearchParcel = intent.getParcelableExtra<SearchEntity>(EXTRA_SEARCH) as SearchEntity
        binding.pbDetail.showPB()

        with(getSearchParcel){
            populateMovies(viewModel.getDetailSearch(id))
    ...
    private fun populateRelate(relateData: LiveData<Resource<PagedList<MoviesEntity>>>) {
        ...
        binding.relateRecycler.layoutManager = LinearLayoutManager(
                this@DetailSearchActivity, LinearLayoutManager.HORIZONTAL, false)
    }

    private fun populateMovies(detailMovies: LiveData<Resource<SearchEntity>>) {
    ...
    private fun setFavorite(data: SearchEntity?) {
        if (data != null) {
            viewModel.setFavoriteSearch(data)
        }
    }
